...
const DealersOrder = () => {
  const [cars, setCars] = useState([]); // Initialize as an empty array
  const [selectedCarId, setSelectedCarId] = useState(""); // State for selected car ID
  const [orders, setOrders] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Fetch cars and store car IDs
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const dealerId = localStorage.getItem('dealerId');
        ...
        setCars(data);
        if (data.length > 0) {
          setSelectedCarId(data[0]._id); // Default to the first car ID if available
        }
  ...
  // Fetch orders for the selected car ID
  useEffect(() => {
    if (!selectedCarId) return; // Exit if no car ID is selected

    const fetchOrders = async () => {
      try {
        const response = await axiosInstance({
          url: `/order/orderss/${selectedCarId}`,
          method: 'GET',
        });
    ...
    fetchOrders();
  }, [selectedCarId]); // Re-fetch orders when selectedCarId changes

  const handleCarChange = (event) => {
    setSelectedCarId(event.target.value);
    setLoading(true); // Set loading state when changing the car
  };
  ...
  return (
    <div>
      ...
      {cars.length > 0 && (
        <div className="mb-6">
          <label htmlFor="carSelect" className="block text-lg font-semibold text-gray-700">Select Car:</label>
          <select
            id="carSelect"
            value={selectedCarId}
